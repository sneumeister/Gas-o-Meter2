...
RTC_DATA_ATTR time_t time_sync_last_epoch = 0;
...
bool time_sync_set_hard(time_t unix_time, const char *source) {
    ...
    time_sync_last_epoch = unix_time;
    ...
    ESP_LOGI(TAG, "Zeit synchronisiert via %s (epoch: %lld)", source ? source : "?",
             (long long)time_sync_last_epoch);
    return true;
}
...
int64_t time_sync_seconds_since_last(void) {
    ...
    return now - time_sync_last_epoch;
}
